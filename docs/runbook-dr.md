...
Если домен настроен и TLS сертификат установлен:

```bash
https://your-domain.com/admin
```

### Через ssh-tunnel (если домен не настроен)

> **DANGER:** Без домена админка использует HTTP. Пароли передаются в открытом виде! Используйте ssh-tunnel для шифрования.

```bash
# На локальной машине создайте tunnel:
...
Трафик между вашей машиной и VPS будет зашифрован через SSH, даже если админка использует HTTP.
