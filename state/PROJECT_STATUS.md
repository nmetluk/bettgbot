...
**Текущая фаза:** Каталог событий работает. Дальше — FSM «Сделать прогноз».
**Реализация:** runtime + конфиг + логгер + модели + миграция + engine + репозитории + внешний реестр + 7 сервисов + aiogram bootstrap + handler `/start` с регистрацией + handler «📅 Все события» (категории/список с пагинацией/карточка); 148 тестов; CI 4 зелёных job'а.

## Где мы сейчас

TASK-001 — TASK-012 закрыты. После TASK-012 в боте работает read-only каталог: `/events` или «📅 Все события» → список категорий с количеством активных событий + pseudo-«🗂 Все категории» → выбор категории → список событий (`PAGE_SIZE=7`, кнопки `‹/›`) → карточка события с описанием, дедлайном, исходами и опциональной строкой «✅ Ваш прогноз». Минимальный `CategoryService` (read-only) + `EventService.list_categories_with_counts` (один SQL с outer join). Типизированные callback'и в `src/bot/callbacks.py` (`CategoryCb`, `EventCb`, `CategoryListCb`).

По итогам review TASK-012 — четыре «keep» и одно «change»: `_check_access` inline-helper выносится в декоратор `@require_active_user` в Step 1 TASK-013 (четвёртое использование шаблона). Следующая задача — TASK-013: FSM «Сделать прогноз» (entry-point из карточки события) — `MakingPrediction.choosing_outcome` → `confirming` → `PredictionService.make_prediction` (upsert, тот же метод работает и для «Изменить прогноз»); декоратор `@require_active_user`; новые callback-data классы; кнопка «🎯 Сделать прогноз» / «✏️ Изменить прогноз» в карточке события появляется только при `can_predict` (дедлайн не прошёл, событие активно).
...
— ничего, ожидание команды на запуск TASK-013.

## Следующие шаги (короткий горизонт)

1. Владелец даёт команду → локальный Claude Code берёт **TASK-013**: FSM «Сделать прогноз» с entry-point из карточки события. Step 1 — декоратор `@require_active_user` в новом `src/bot/auth.py` (заменяет `_check_access` inline во всех 4 handler'ах `events.py`). FSM `MakingPrediction.choosing_outcome` → `confirming`. Новые callback-data классы (`PredictStartCb`, `PredictPickCb`, `PredictConfirmCb`, `PredictCancelCb`). Расширение `event_card_kbd` — кнопка «🎯 Сделать прогноз» / «✏️ Изменить прогноз» только при `can_predict`. Кнопка главного меню «🎯 Сделать прогноз» / `/predict` — переадресация в `cmd_events`.
2. После TASK-013 — TASK-014: команда «Мои прогнозы» (активные + архив + статистика пользователя). Возможный вынос `PAGE_SIZE` в `src/bot/_consts.py`, если повторится.
3. После TASK-014 — TASK-015: команда «Настройка напоминаний» (FSM добавления/удаления интервалов, пресеты + свой ввод).
