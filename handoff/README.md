...
## Когда задача считается закрытой (для локального агента)

🚨 **Задача НЕ закрыта, пока CI не зелёный и PR не смёрджен в `main`.** Нельзя архивировать
задачу, писать «всё готово / Открытые вопросы: Нет» и переходить к следующей, если:

- хоть одна проверка CI красная (`ruff`, `mypy`, `pytest`, `handoff-consistency`, security-jobs);
- ветка запушена, но PR **не смёрджен** в `main`;
- `pytest` локально не прогнан после изменений.

Минимальный чек-лист закрытия:

1. `uv run pytest` — **зелёный полностью** (не только затронутый поднабор).
2. `ruff check` / `ruff format --check` / `mypy` — чисто.
3. PR открыт, **CI зелёный**, ветка **смёрджена** (squash); локальная `main` синхронизирована.
4. Отчёт в `handoff/outbox/TASK-NNN-report.md` написан.
5. Задача заархивирована **директорией** `handoff/archive/TASK-NNN-<slug>/task.md` (не плоским файлом).

**Если работа сделана, но CI красный или PR не смёрджен — это незакрытая задача.** Так и писать
в отчёте: что готово, что краснит, что мешает merge. Не помечать «готово».

> Контекст (повторяющиеся инциденты): при реверстке шаблонов несколько раз менялся текст
> (`«Дашборд» → «Главная»`, `«Журнал аудита» → «Аудит-лог»`, разметка ячеек), но текстовые
> ассерты в тестах не обновлялись — CI краснел, а задача помечалась «готово». **Правило: меняешь
> текст/разметку в шаблоне — синхронно обновляй текстовые ассерты в тестах** (`in response.text`),
> логику при этом не трогая.
