...
**Текущая фаза:** Persistence слой готов. Следующее — query-слой (репозитории).
**Реализация:** runtime-инфра + конфиг + логгер + 8 ORM-моделей + миграция `0001_init` + async engine; 22 unit + 4 integration теста; CI 4 зелёных job'а.

## Где мы сейчас

TASK-001 — TASK-006 закрыты. БД-схема развёрнута миграцией `0001_init` (8 таблиц + 24 индекса + CHECK constraints + циклическая FK через `use_alter`); `src/shared/db.py` экспортирует async engine, `async_sessionmaker(expire_on_commit=False)` и `get_session` async-генератор для DI. Alembic с async-`env.py` читает URL из `Settings`. Makefile: `make migrate`, `rollback`, `migration.new MSG="..."`. Новый CI job `integration` поднимает postgres:16 service-container и применяет миграцию на чистую БД.

Следующая задача — TASK-007: 8 репозиториев в `src/shared/repositories/` (тонкий query-слой, AsyncSession в конструкторе, NO commit inside), integration-тесты с fixture-session (rollback после теста).
...
— ничего, ожидание команды на запуск TASK-007.

## Следующие шаги (короткий горизонт)

1. Владелец даёт команду → локальный Claude Code берёт **TASK-007**: 8 репозиториев в `src/shared/repositories/`, минимум методов под предстоящие фичи, integration-тесты с fixture-session (rollback).
2. После TASK-007 — TASK-008: сервисы (`UserService`, `EventService`, `PredictionService`, `StatsService`, `ReminderService`, `AuditService`) — композиция репозиториев, транзакции, доменные исключения.
3. После TASK-008 — TASK-009: интерфейс `ExternalUserRegistryClient` + `MockExternalUserRegistryClient` (по [`docs/06-external-api.md`](../docs/06-external-api.md)).
