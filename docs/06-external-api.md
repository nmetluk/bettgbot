# 06 — Внешний API проверки пользователей (ОТМЕНЁН)

> 🚫 **SUPERSEDED — 2026-06-01, [ADR-0006](adr/0006-open-registration.md).**
> Заказчик отменил проверку пользователей по внешнему реестру. Регистрация теперь **открытая**: любой, кто поделился своим контактом, регистрируется ([04-bot-flows.md](04-bot-flows.md)).
>
> Код реестра (`src/shared/external/`, HTTP- и mock-клиенты), `verify`-вызов в `UserService`, исключения `UserNotAllowed`/`RegistryUnavailableError`/`ExternalApiError`, связанные тексты и env-переменные **удаляются** — см. [TASK-096](../handoff/archive/).
>
> Документ ниже сохранён только как историческая справка о ранее предлагавшемся контракте на случай, если проверку вернут.
...
Регистрация в боте была разрешена только для пользователей, чьи телефоны уже есть во внешнем реестре. Сами мы пользователей в реестр не заводили — только проверяли. **С 2026-06-01 это правило отменено** (см. баннер выше).
