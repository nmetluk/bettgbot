...
- [ ] **Переиспользование стилей:** общие компонентные классы (`.pv-table`, `.pv-badge`, `.pv-card`, `.pv-list-row` и т. п.), которые в TASK-054 попали inline в `dashboard.html`, вынесены в `src/admin/static/css/app.css` и переиспользуются здесь; новые таблицы/бейджи **не** дублируют CSS inline. Inline допустим только для действительно уникальных для экрана стилей.
