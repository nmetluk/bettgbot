...
**Текущая фаза:** **Этап 2 (Telegram-бот) почти закрыт — все 7 пользовательских handler'ов готовы.** Дальше — фоновые задачи (APScheduler).
**Реализация:** runtime + конфиг + логгер + модели + миграция + engine + репозитории + внешний реестр + 7 сервисов + aiogram bootstrap + **6 реальных router'ов (start / events / prediction / my / reminders / help)** + декоратор `@require_active_user` + 2 FSM (`MakingPrediction`, `EditingReminders`) + helper `render_event_card` + parser `parse_offset`; ~224 теста (149 unit + 4 migrations + 36 repos + 35 services); CI 4 зелёных job'а; зеркало в Google Drive через MCP-коннектор работает.

## Где мы сейчас

TASK-001 — TASK-016 закрыты. Бот делает **полную пользовательскую поверхность**:

- **Регистрация** (`/start` + Contact) → mock-реестр → `User` + дефолтный `ReminderSetting([1440, 60])`.
- **Каталог событий** (`/events`) → категории → события → карточка с кнопкой прогноза.
- **FSM «Сделать прогноз»** → upsert через `PredictionService.make_prediction`.
- **«Мои прогнозы»** (`/my`) → таб «🟢 Активные» / «📦 Архив», тап → карточка, статистика под архивом.
- **«🔔 Напоминания»** (`/reminders`) → toggle + 6 пресетов + FSM свободного ввода + удаление.
- **`/help`** + кнопка «ℹ️ Справка» → статический текст + главное меню.

Все доменные исключения теперь с типизированным `reason: Literal[...]` (`EventNotPredictableError`, `InvalidReminderOffsetsError`), handler'ы ловят через `match exc.reason: case ...` — mypy validate exhaustive.

По итогам review TASK-016 — **все 3 решения «keep»** (никаких code changes). Стилевая норма handler-сигнатур закреплена.

Следующая задача — **TASK-017: APScheduler-job рассылки напоминаний**. Первая фоновая задача — потребует:
- Новой модели `ReminderDispatchLog(user_id, event_id, offset_minutes, dispatched_at)` + миграция 0002 (уникальный индекс для дедупликации).
- Расширения `ReminderService.find_candidates(now, window_minutes)` — один SQL с join'ами `event × reminder_setting × user`, фильтрация по дедлайну/offset'ам/отсутствию prediction/отсутствию dispatch_log.
- `src/bot/scheduler/` — `AsyncIOScheduler`, фабрика `build_scheduler(bot, session_maker)`, job каждые 5 минут.
- Интеграция в `src/bot/main.py` — scheduler стартует параллельно polling.
- Текст `REMINDER_NOTIFICATION` с deeplink на /events.

Размер L (>4h), потому что это новая модель + миграция + новый сервис-метод + scheduler infra + интеграция + integration-тесты.
...
- 2026-05-24 — сессия приёмки `2026-05-24-02-task-016-review`; все 3 решения «keep» (noqa-suppressions не нужны без enabled-правила; сигнатура `cmd_help(user, session)` — стилевая норма однородных handler-сигнатур; integration-тест на `exc.reason` — opt-in, добавим при следующем касании reminder-тестов)

## Что в работе прямо сейчас

— ничего, ожидание команды на запуск TASK-017.

## Следующие шаги (короткий горизонт)

1. Владелец даёт команду → локальный Claude Code берёт **TASK-017**: APScheduler-job рассылки напоминаний (первая фоновая задача). **Step 1**: новая модель `ReminderDispatchLog(user_id, event_id, offset_minutes, dispatched_at)` с уникальным индексом `(user_id, event_id, offset_minutes)` + миграция 0002. **Step 2**: `ReminderDispatchLogRepository` (`was_dispatched`, `record` с `on_conflict_do_nothing`). **Step 3**: метод `ReminderService.find_candidates(now, window_minutes)` — один SQL с join'ами event × reminder_setting × user, фильтрация по дедлайну/offset'ам/отсутствию prediction и dispatch_log. **Step 4**: `src/bot/scheduler/` — `AsyncIOScheduler`, фабрика `build_scheduler(bot, session_maker)`, job каждые 5 минут. **Step 5**: интеграция в `main.py`. **Step 6**: текст `REMINDER_NOTIFICATION` + deeplink. **Step 7**: integration-тесты на `find_candidates` (5+ сценариев) + unit на job + smoke на scheduler bootstrap. Размер L.
2. После TASK-017 — **TASK-018**: APScheduler-job архивации событий после фиксации итога (быстрый путь) или по таймауту 7 дней без итога (страховка). Закрывает Этап 2.
3. После TASK-018 → Этап 3 — веб-админка. **TASK-019** скелет FastAPI + Jinja2 + Bootstrap 5 (выбор готового шаблона — AdminLTE / SB Admin 2 / Volt — решается в самой задаче).
