...
erDiagram
    ...
    EVENT {
        ...
        timestamptz result_notified_at "nullable, когда бот разослал админам пост-итоговую статистику"
        timestamptz created_at
...
Telegram-пользователь, поделившийся своим контактом. Регистрация открытая ([ADR-0006](adr/0006-open-registration.md)).

- `tg_user_id` — уникальный, по нему мы находим пользователя при любом апдейте.
- `phone` — в формате E.164, без пробелов и скобок. Уникальный. Хранится для дозвона/связи.
- `is_blocked` — флаг для блокировки доступа из админки без удаления.
- При первом контакте пользователя бот **создаёт** запись сразу, как только получен **свой** контакт. Никакой проверки во внешнем реестре нет (отменена 2026-06-01). Полей, завязанных на внешний реестр (`external_user_id` и т. п.), в таблице нет — миграции не требуются.
...
- `result_notified_at` (nullable, TASK-097) — момент, когда бот разослал админам пост-итоговую статистику события. `NULL` = ещё не разослано. Бот-джоб `dispatch_event_result_notifications` выбирает события `result_outcome_id IS NOT NULL AND result_notified_at IS NULL` и после успешной рассылки ставит `now()`. Поле обеспечивает идемпотентность (повторно не уйдёт) и развязку процессов web↔bot. Не входит в CHECK-инвариант ниже (независимо от состояния архивации).
...
- `category_id` — обязательно при `segment='category'` (CHECK constraint `ck_broadcast_category_required`).
- `message_text` — текст сообщения (плоский, без HTML; ≤ 4096 байт — лимит Telegram).
- **Удаление Category**: запрещено (FK RESTRICT), если существует Broadcast со ссылкой на эту категорию (сегментные рассылки сохраняют историческую привязку). Сервис поднимает `CategoryHasBroadcastsError` (TASK-085). Категорию можно деактивировать (`is_active=false`), но не удалять при наличии исторических анонсов.
...
- **Жёсткое удаление** разрешено только для `Category` (если в ней нет событий **и** нет рассылок с `segment='category'`, см. `CategoryHasEventsError` / `CategoryHasBroadcastsError` в TASK-085) и для `Outcome` (если на него нет прогнозов).
