...
**Текущая фаза:** Доменный слой готов. Следующее — engine, sessions, миграции.
**Реализация:** runtime-инфра + конфиг + логгер + 8 ORM-моделей; 22 unit-теста; CI зелёный.

## Где мы сейчас

TASK-001 — TASK-005 закрыты. Доменный слой описан декларациями SQLAlchemy 2.0 в `src/shared/models/` (`User`, `Category`, `Event`, `Outcome`, `Prediction`, `ReminderSetting`, `AdminUser`, `AuditLog`); `Base.metadata` с naming convention и `type_annotation_map`; партиальные индексы, CHECK на инвариантах события, циклика `Event ↔ Outcome` через `use_alter`. По итогам review TASK-005 согласованы три минорные правки моделей — встроены в DoD TASK-006 (применяются до автогенерации миграции).

Следующая задача — TASK-006: применить три tweaks из review TASK-005 → `src/shared/db.py` (async engine + `async_sessionmaker` + `get_session`) → Alembic init с async-`env.py` → автогенерация и вычитка первой миграции `0001_init.py` → Makefile targets → integration-тест применения миграции против Postgres-service в CI.
...
— ничего, ожидание команды на запуск TASK-006.

## Следующие шаги (короткий горизонт)

1. Владелец даёт команду → локальный Claude Code берёт **TASK-006**: tweaks моделей → `db.py` → Alembic async-env → `0001_init` → Makefile targets → integration-тест в CI.
2. После TASK-006 — TASK-007: репозитории (`UserRepository`, `EventRepository`, `PredictionRepository`, `AuditRepository`) — тонкий слой запросов на агрегат.
3. После TASK-007 — TASK-008: интерфейс `ExternalUserRegistryClient` + `MockExternalUserRegistryClient` (по [`docs/06-external-api.md`](../docs/06-external-api.md)).
