...
**Текущая фаза:** Query-слой готов. Следующее — внешний реестр (интерфейс + mock) перед сервисами.
**Реализация:** runtime-инфра + конфиг + логгер + 8 ORM-моделей + миграция + async engine + 8 репозиториев; 59 тестов (22 unit + 4 migrations + 33 repositories); CI 4 зелёных job'а.

## Где мы сейчас

TASK-001 — TASK-007 закрыты. Доменный + persistence + query слои готовы. 8 репозиториев в `src/shared/repositories/` (один на агрегат, `AsyncSession` в конструкторе, без бизнес-логики и commit/rollback); `pg_insert + on_conflict_do_update` для upsert с `refresh()`; `mark_correctness` через `update + case`; `users_without_prediction_for_event` через `NOT EXISTS`. Integration-тесты с per-test engine + NullPool (обход «Event loop is closed» pytest-asyncio). Unit CI job сужен до `tests/unit/`.

По итогам review TASK-007 порядок ближайших задач изменён: сервисы зависят от `ExternalUserRegistryClient`, поэтому внешний реестр идёт **раньше** сервисов. Следующая задача — TASK-008: интерфейс + mock + http-каркас внешнего реестра пользователей по [`docs/06-external-api.md`](../docs/06-external-api.md).
...
— ничего, ожидание команды на запуск TASK-008.

## Следующие шаги (короткий горизонт)

1. Владелец даёт команду → локальный Claude Code берёт **TASK-008**: `src/shared/external/{registry.py, mock_registry.py, http_registry.py, __init__.py}` по [`docs/06-external-api.md`](../docs/06-external-api.md); unit-тесты на mock и http (через `httpx.MockTransport`).
2. После TASK-008 — TASK-009: сервисы (`UserService` использует `ExternalUserRegistryClient`, `EventService`, `PredictionService`, `StatsService`, `ReminderService`, `AuditService`) — композиция репозиториев, транзакции, доменные исключения.
3. После TASK-009 — TASK-010: скелет aiogram-приложения (bootstrap, диспетчер, middleware с `get_session` и `touch_last_seen`, логирование).
