...
**Задача:** `handoff/inbox/TASK-068.in-progress.md` + `handoff/inbox/TASK-068-amendment.in-progress.md`
**PR:** https://github.com/nmetluk/bettgbot/pull/124

## Кратко

Задача выполнена полностью (основной фикс + amendment). CSRF-кука больше не ротируется на каждый GET-запрос, что позволило исправить блокер — **пользователи теперь могут залогиниться в админку**.

**Основной фикс:** `CsrfTokenMiddleware` больше не генерирует новую пару CSRF-токенов на каждый GET. Если валидная CSRF-кука уже есть — middleware декодирует unsigned-токен через `URLSafeTimedSerializer` и отрисовывает форму с этим токеном, НЕ перезаписывая куку.

**Amendment:** Добавлен явный TTL CSRF-токена (15 минут) и подсказка в футере login.html на случай если вход не удался.

## Что сделано

### 1. `src/admin/auth/middleware.py` (основной фикс)
...
### 2. `src/admin/routes/login.py` (основной фикс)
...
### 3. `src/admin/app.py` (amendment)

- В `_csrf_config()` добавлен явный `max_age=900` (15 минут).
- После фикса ротации именно этот TTL становится реальным сроком жизни токена — отвлёкшийся пользователь (до 15 минут) не выпадает из сессии.

### 4. `src/admin/templates/login.html` (amendment)

- В футер добавлена подсказка: «Если вход не удался — обновите страницу и попробуйте снова».
- Согласовано с текстом ошибки «Сессия истекла, обновите страницу.» — формулировки не противоречат.
