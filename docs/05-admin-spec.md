...
без собственных тем). Выбор стека для v2 — открытое решение (см. `decisions.md` сессии, п. 4);
при переходе на React потребуется отдельный ADR, заменяющий соответствующую часть этого документа.
