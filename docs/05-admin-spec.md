...
- **Server-side rendering.** HTML отдаётся сервером, точечные обновления — через HTMX. Никакого SPA.
- **Готовый шаблон.** На старте используется один из публичных Bootstrap 5 админ-темплейтов (выбор в TASK-018: кандидаты — [AdminLTE 4](https://adminlte.io/), [Volt Free](https://demo.themesberg.com/volt/), [SB Admin 2](https://startbootstrap.com/theme/sb-admin-2)). Никаких собственных дизайн-токенов и тем.
...
> ⚠️ **Статус — референс, не утверждённая спецификация.** Дизайн расходится с текущим разделом.
...
**Важно:** прототип построен как клиентский SPA на React с собственными дизайн-токенами, что
противоречит зафиксированному выше принципу SSR (FastAPI + Jinja2 + HTMX + Bootstrap 5, без SPA,
без собственных тем). Выбор стека для v2 — открытое решение: разбор вариантов и рекомендация
архитектора в [ADR 0005 — Стек веб-админки v2](adr/0005-admin-v2-stack.md) (Status: Proposed).
До его принятия задачи на реализацию v2 не заводятся.
